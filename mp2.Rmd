...
We took information from the Federal Election Committee (FEC) to look at the ratios between contributions for and against candidates. The FEC provides information every election cycle on the candidates, the contributions of committees, and the committees making contributions. In the 2011-2012 general election cycle for the House of Representatives, we looked at the contributions for and against the candidates. Since some transactions are for a certain candidate and other transactions are against a certain candidate, we wanted to know what the ratio was between money given for the election of the winning candidate, in contrast with the money given against the election of the losing candidate. Essentially, if any committee wants Joe Smith to win in a certain district, what is the winning combination of donations for Joe Smith and against Smith's opponent? 
...
general_house <- house_elections %>%
  filter(general_votes != 0) #filters the house_elections data down to only the general elections
...
#filters so that the candidates table only includes candidates running for the house of representatives
```
...
#joins general_house and house_candidates to connect information about election results
```
...
#joins small_house and contributions to connect election results, candidates, and contributions
```

```{r}
ca_house_contributions <- small_house_contributions %>%
  filter(state.x == "CA", transaction_type == "24A" | transaction_type == "24E")
#brings the house contributions table down to California, and only transactions of type 24A and 24E
```
...
#plots the data so that each district's bar is the same height and the division between the two colors represents the proportion for each of the two categories
```
This graph and previous code showes the summation of of our work towards wrangling the data we wanted out of the original two tables, specifically using looking at California.  The following code shows how we created a function to apply this to all states.
...
By using past trends to predict the future, we can learn where to spend money to make it most effective in getting your desired outcome.
...
For example, consider New York. Between the two categories of contribution in consideration, over 75% of money spent in each of those individual district campaigns were spend 'against the loser'. Clearly, those New York voters are more receptive to negative propaganda than they are to positive.
=======
Take New York for example.  Most of the money spent in each of those individual district campaigns were spend "against the loser".  Clearly, those New York voters are more receptive to negative propaganda than they are to positive.
>>>>>>> 1496178bae61881986eb72ea82a35101763d24e7
