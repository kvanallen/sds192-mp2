...
total_amount_tbl <- ca_house_contributions %>%
  ...
  summarize(total_amount = sum(transaction_amt)) #condenses ca_house_contributions so that there is one row per candidate per contribution type
```

```{r}
small_ca_contributions <- ca_house_contributions %>%
  full_join(total_amount_tbl, by = c("candidate_name" = "candidate_name")) %>%
  select(fec_id, state.x, district, incumbent, candidate_name, party, general_votes, ge_winner, transaction_type.y, total_amount) #joins ca_house_contributions with total_amount_tbl to get the information of the former with the total from the latter
```
